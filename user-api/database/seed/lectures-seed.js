...
module.exports.seedLectures = async function seedLectures(
  ...
  for (let i = 0; i < insertedTeachers.length; i++) {
    ...
    for (let j = 0; j < numberOfLecturesPrTeacher; j++) {
        ...
        lectureNames = ['Large System Development', 'Test', 'System Integration', 'Data Science', 'Math and Algorithms', 'Exploration and Presentation', 'Databases for Developers', 'Finance', 'Architechture', 'Innovation', 'Business Modelling', 'Business Engineering', 'Web-design', 'Entrepreneurship', 'Business Management', 'Programming', 'English', 'Math', 'Physics', 'Chemistry', 'Microbiology', 'Electronic Engineering'];

        const lecName = lectureNames[Math.floor(Math.random() * lectureNames.length)];

      const studentsPrTeacher = parseInt(insertedStudents.length / insertedTeachers.length);
